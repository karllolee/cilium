...
func (k *SourceRangeKey4) GetCIDR() *net.IPNet {
	var (
		cidr net.IPNet
		ip   types.IPv4
	)
	cidr.Mask = net.CIDRMask(int(k.PrefixLen)-lpmPrefixLen4, 32)
...
func (k *SourceRangeKey6) GetCIDR() *net.IPNet {
	var (
		cidr net.IPNet
		ip   types.IPv6
	)
	cidr.Mask = net.CIDRMask(int(k.PrefixLen)-lpmPrefixLen6, 128)
	k.Address.DeepCopyInto(&ip)
	cidr.IP = ip.IP()
	return &cidr
}
...
func srcRangeKey(cidr *net.IPNet, revNATID uint16, ipv6 bool) bpf.MapKey {
